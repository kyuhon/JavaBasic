...
public class Stream3Main {
	...
	public static void main(String[] args) {
		...
		Stream <Integer> stream4 = stream3.map(String::length);
		
		// 4. 스트림을 종단처리(배열이나, 컬렉션프레임으로 변경)
		List<Integer> list2 = Arrays.stream(name).filter(str -> str.startsWith("C"))
				.sorted((o1,o2) -> o1.compareTo(o2)).map(t -> t.length()).collect(Collectors.toList());
